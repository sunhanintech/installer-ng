...
mysql_database 'load scalr database structure' do
  ...
  database_name   node[:scalr][:database][:scalr_dbname]
  sql             { ::File.open("#{node[:scalr][:core][:location]}/sql/structure.sql").read }
  not_if          { mysql_has_table?(mysql_root_params(node), node[:scalr][:database][:scalr_dbname], canary_table) }
  action          :query
end
...
mysql_database 'load scalr database data' do
  ...
  not_if          { mysql_has_rows?(mysql_user_params(node), node[:scalr][:database][:scalr_dbname], canary_table) }
  action          :query
end
...
if has_cost_analytics? node
  mysql_database 'load analytics database structure' do
    connection      mysql_user_params(node)
    database_name   node[:scalr][:database][:analytics_dbname]
    sql             { ::File.open("#{node[:scalr][:core][:location]}/sql/analytics_structure.sql").read }
    not_if          { mysql_has_table?(mysql_root_params(node), node[:scalr][:database][:analytics_dbname], 'upgrades') }
    action          :query
  end
  ...
  mysql_database 'load analytics database data' do
    ...
    not_if          { mysql_has_rows?(mysql_user_params(node), node[:scalr][:database][:analytics_dbname], 'upgrades') }
    action          :query
  end
end
