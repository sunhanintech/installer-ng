...
mysql_database 'grant_replication_permissions' do
  ...
  connection      mysql_admin_params(node)
  database_name   'mysql'
  sql             "GRANT REPLICATION SLAVE ON *.* TO '#{node[:scalr_server][:mysql][:repl_user]}'@'#{node[:scalr_server][:mysql][:repl_allow_connections_from]}' WITH GRANT OPTION; FLUSH PRIVILEGES;"
  action          :query
end
