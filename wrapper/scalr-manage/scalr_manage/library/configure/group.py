...
class AppGroup(Group):
    ...
    def make_attributes(cls, args, ui, tokgen):
        ...
        if urlparse.urlparse(repo).scheme in constant.GIT_NON_SSH_SCHEMES:
            logger.debug("You will not need a SSH key for this repository ({0}).".format(repo))
            ssh_key = ""
            ssh_key_path = ""
        else:
        ...
        if not MysqlGroup.is_enabled(args):
            logger.info("No database will be installed. Provide database configuration details.")
